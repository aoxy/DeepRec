...
void to_array(int64* ids, size_t ids_idx, int64* batch_ids, size_t batch_size) {
  ...
  for (size_t i = 0; i < batch_size; ++i) batch_ids[i] = ids[i + ids_idx];
}
...
void RemoveCache(BatchCache<int64>* cache, size_t capacity, size_t batch_size,
  ...
  while (*insert_flag < add_workers_num) {
    ...
    sleep_us = std::max(sleep_us, (size_t)1000);
    usleep(sleep_us);
  }
  delete[] evic_ids;
...
class dataloder {
 ...
 public:
  ...
  dataloder(std::string filepath, size_t feature_id, size_t max_id_num = 0) {
    ...
    if (!fp) {
      LOG(INFO) << "open" << filepath << "fail." << std::endl << std::flush;
      exit(1);
    }
    std::string line;
    std::getline(fp, line);
    while (std::getline(fp, line)) {
      ...
      s.insert(id);
      ids.push_back(id);
      if (max_id_num > 0 && ids.size() >= max_id_num) break;
    }
  }
...
void TestCacheOnTaoBao(dataloder* dl, BatchCache<int64>* cache,
  ...
  uint64 end = Env::Default()->NowNanos();
  uint64 result_cost = end - start;
  if (typeid(*cache) == typeid(BlockLockLFUCache<int64>)) {
    LOG(INFO) << typeid(*cache).name() << "(way=" << way
              << ") Cost Time: " << result_cost / 1000000 << " ms";
  } else {
    LOG(INFO) << typeid(*cache).name()
              << " Cost Time: " << result_cost / 1000000 << " ms";
  }
...
TEST(EmbeddingVariableTest, TestCacheTaoBaoBatch) {
  ...
  for (size_t i = 0; i < thread_nums.size(); i++) {
    ...
    std::vector<BatchCache<int64>*> caches;
    for (size_t j = 0; j < way_nums.size(); j++) {
      caches.push_back(new BlockLockLFUCache<int64>(capacity, way_nums[j]));
    }

    TestCacheOnTaoBao(&dl, cache1, thread_nums[i], batch_size, capacity, 0);
    for (size_t j = 0; j < way_nums.size(); j++) {
      TestCacheOnTaoBao(&dl, caches[j], thread_nums[i], batch_size, capacity,
                        way_nums[j]);
    }
  }
}
...
TEST(EmbeddingVariableTest, TestSimpleCalculation) { SimpleCalculation(); }
